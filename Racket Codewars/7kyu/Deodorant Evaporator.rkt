...
(define (evaporator content evap-per-day threshold)
  (define threshold-content (* content (/ threshold 100)))
  (define (helper current-content days)
    (if (< current-content threshold-content)
        days
        (helper (* current-content (- 1 (/ evap-per-day 100))) (+ days 1))))
  (helper content 0))