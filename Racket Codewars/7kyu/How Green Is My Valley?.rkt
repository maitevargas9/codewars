...
difference l - r being as small as possible. In other words the right wing must be nearly as steep as 
the left wing.
...
(define (make-valley lst)
  ...
  (define-values (left right _)
    (for/fold ((left '()) (right '()) (i 0))
              ((x sorted))
      (if (even? i)
          (values (cons x left) right (+ i 1))
          (values left (cons x right) (+ i 1)))))
  (append (reverse left) (sort right <)))