...
this number has at most 2 digits because you are supposed to know if a number of at most 2 digits 
...
function (this last m will be divisible or not by 7), the second one being the number of steps to get the result.
...
(define (seven m)
  (let loop ([cnt 0] [m m])
    (if (< m 100)
      (cons m cnt)
      (loop (add1 cnt) (- (quotient m 10) (* 2 (modulo m 10)))))))