...
Note: The original string should be included in the output array. The order matters. Each element of the output array 
should be the rotated version of the previous element. The output array SHOULD be the same length as the input string. 
...
public class ScrollingTextMachine {
  ...
  public String[] rotate(String text) {
    ...
    for (int i = 0; i < n; i++) {
      rotations[i] = text.substring(i + 1) + text.substring(0, i + 1);
    }
    return rotations;
  }
}
