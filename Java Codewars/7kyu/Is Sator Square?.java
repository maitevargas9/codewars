...
public class Kata {
  ...
  public static Boolean isSatorSquare(char[][] tablet) {
    ...
    for (int i = 0; i <= l / 2; i++) {
      int j = l - 1 - i;
      String across = new String(tablet[i]);
      StringBuilder downBuilder = new StringBuilder();
      for (int k = 0; k < l; k++) {
        downBuilder.append(tablet[k][i]);
      }
      String down = downBuilder.toString();
      StringBuilder upBuilder = new StringBuilder();
      for (int k = 0; k < l; k++) {
        upBuilder.append(tablet[k][j]);
      }
      String up = upBuilder.reverse().toString();
      String reverse = new StringBuilder(new String(tablet[j]))
        .reverse()
        .toString();
      if (
        !(across.equals(down) && across.equals(up) && across.equals(reverse))
      ) {
        return false;
      }
    }
