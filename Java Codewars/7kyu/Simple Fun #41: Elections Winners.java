...
Given an array of numbers representing votes given to each of the candidates, and an integer which is equal to the number of 
voters who haven't cast their vote yet, find the number of candidates who still have a chance to win the election.
The winner of the election must secure strictly more votes than any other candidate. If two or more candidates receive the same 
...
class ElectionWinners {
  ...
  static int find(final int[] votes, final int k) {
    int max = Arrays.stream(votes).max().orElse(0);
    if (k > 0) {
      return (int) Arrays.stream(votes).filter(v -> v + k > max).count();
    } else {
      long maxCount = Arrays.stream(votes).filter(v -> v == max).count();
