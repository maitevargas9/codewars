...
# They can be directed in the same or opposite directions. One vector can be obtained from another by multiplying it by a 
# certain number. In terms of coordinates, vectors (x1, y1) and (x2, y2) are collinear if (x1, y1) = (k*x2, k*y2) , 
# where k is any number acting as a coefficient.
# For more information, check out this article on collinearity.
# Problem Description
# Write the function collinearity(x1, y1, x2, y2), which returns a Boolean type depending on whether the vectors are collinear 
# or not.
...
collinearity <- function(x1, y1, x2, y2) {
  return(x1 * y2 == y1 * x2)
}