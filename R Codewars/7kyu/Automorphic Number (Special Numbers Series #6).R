# Description
# A number is called Automorphic number if and only if its square ends in the same digits as the number itself. For example, 25 is an 
...
automorphic <- function(n) {
  ...
  if (endsWith(as.character(sq), as.character(n))) {
    return("Automorphic")
  } else {
    return("Not!!")
  }
