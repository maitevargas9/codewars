...
# You will be given two strings: lamps and drone. lamps represents a row of lamps, currently off, each represented by 
# x. When these lamps are on, they should be represented by o.
...
fly_by <- function(lamps, drone) {
  lit_count <- nchar(drone)
  result <- paste0(strrep("o", lit_count), substr(lamps, lit_count + 1, nchar(lamps)))
  return(result)
}