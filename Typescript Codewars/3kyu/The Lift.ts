...
export const theLift = (queues: number[][], capacity: number): number[] => {
  ...
  while (lift.length || queues.filter((x) => x.length).length) {
    let onWay = queues[floor].filter((y) =>
      direction == 1 ? y > floor : y < floor
    );
    if (lift.includes(floor) || onWay.length) {
      if (visitedFloors[visitedFloors.length - 1] != floor) {
        visitedFloors.push(floor);
      }
      lift = lift.filter((z) => z != floor);
      queues[floor] = queues[floor].filter(
        (a) => lift.length == capacity || !onWay.includes(a) || !lift.push(a)
      );
    }
    floor += direction;
    direction = +!floor || -(floor == queues.length - 1) || direction;
  }
