...
Vowel harmony is a phenomenon in some languages. It means that "A vowel or vowels in a word are changed to sound 
the same (thus "in harmony.")" (wikipedia). This kata is based on vowel harmony in Hungarian.

Task:
Your goal is to create a function instrumental() which returns the valid form of a valid Hungarian word w in 
instrumental case i. e. append the correct suffix -vel or -val to the word w based on vowel harmony rules.
...
export function instrumental(word: string): string {
  ...
  if (/[eéiíöőüűaáoóuú]$/.test(word)) {
    word = word.replace(/[aeiouöü]$/, (c) => pairs[c] || c) + "v";
