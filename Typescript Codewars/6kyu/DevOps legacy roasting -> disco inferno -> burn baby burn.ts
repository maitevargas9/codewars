...
You will receive a string of workloads represented by words….some legacy and some modern mixed in with complaints 
from the business….your job is to burn the legacy in a disco inferno and count the value of each roasting and the 
...
Disclaimer - this should only be attempted by trained professionals and in accordance with local ordinances. 
...
export function roastLegacy(workloads: string): string {
  ...
  let points = 0;
  workloads = workloads.toLowerCase();
  for (let i in keywords) {
    if (workloads.split(i).length > 0) {
      points += (workloads.split(i).length - 1) * keywords[i];
    }
  }
  for (let i of complaints) {
    if (workloads.includes(i)) {
      countComplaints += workloads.split(i).length - 1;
    }
  }
  return points == 0 && countComplaints == 0
    ? "These guys are already DevOps and in the Cloud and the business is happy!"
    : `Burn baby burn disco inferno ${points} points earned in this roasting and ${countComplaints} complaints resolved!`;
}
