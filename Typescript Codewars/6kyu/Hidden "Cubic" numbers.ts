...
The task is to find, or not, the "cubic" numbers in the string and then to make the sum of these "cubic" numbers found in the string, if any, 
...
s = "QK29a45[&erui9026315" the groups are 29, 45, 902, 631, 5. None is cubic. Return: "Unlucky"
...
When a continuous string of digits exceeds 3, the string is split into groups of 3 from the left. The last grouping could have 3, 2 or 1 digits. e.g "24172410" becomes 3 strings comprising "241", "724" and "10" e.g "0785" becomes 2 strings comprising "078" and "5".
*/
...
export function isSumOfCubes(s: string): string {
  ...
  for (const group of digitGroups) {
    for (let i = 0; i < group.length; i += 3) {
      const chunk = group.slice(i, i + 3);
      const num = Number(chunk);

      const sumOfCubes = chunk
        .split("")
        .reduce((sum, d) => sum + Math.pow(Number(d), 3), 0);
  ...
  const sum = cubicNumbers.reduce((a, b) => a + b, 0);
