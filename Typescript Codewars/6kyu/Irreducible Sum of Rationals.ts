...
or
lst = [ (numer_1, denom_1) , ... , (numer_n, denom_n) ]
where all numbers are positive integers. You have to produce their sum N / D in an irreducible form: this means that N and D 
...
export function sumFracts(l: number[][]): string | null {
  ...
  let total = [0, 1];
  for (let item of l) {
    total[0] = total[0] * item[1] + total[1] * item[0];
    total[1] = total[1] * item[1];
    let g = gcd(total[0], total[1]);
    total[0] /= g;
    total[1] /= g;
  }
