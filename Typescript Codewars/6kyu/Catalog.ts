...
export function catalog(s: string, article: string): string {
  const regex = new RegExp(`name.+${article}.+/qty`, "g");
  const matches = s.match(regex);
  return matches
    ? matches
        .map((x: string) => x.replace(/<\/name><prx>/, " > prx: $"))
        .map((x: string) => x.replace(/name>/, ""))
        .map((x: string) => x.replace(/<\/prx><qty>/, " qty: "))
        .map((x: string) => x.replace(/<\/qty/, ""))
        .join("\r\n")
    : "Nothing";
}
