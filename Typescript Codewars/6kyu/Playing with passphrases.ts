...
to common cultural references. You can get your passphrases stronger by different means. One is the following:
...
export function playPass(s: string, n: number): string {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (char >= "A" && char <= "Z") {
      ...
      transformed +=
        i % 2 === 0 ? shifted.toUpperCase() : shifted.toLowerCase();
    } else if (char >= "0" && char <= "9") {
      transformed += String(9 - Number(char));
    } else {
      transformed += char;
    }
  }
