...
export function triangle(row: string): string {
  const alphabetValue = (char: string): number => char.charCodeAt(0) - 96;
  const valueToChar = (val: number): string =>
    String.fromCharCode(((val - 1) % 26) + 97);

  while (row.length > 1) {
    let nextRow = "";
    for (let i = 0; i < row.length - 1; i++) {
      const sum = alphabetValue(row[i]) + alphabetValue(row[i + 1]);
      nextRow += valueToChar(sum);
    }
    row = nextRow;
  }
