...
between these towns. ls = [50, 55, 57, 58, 60]. John is tired of driving and he says to Mary that he doesn't 
...
all distances are positive or zero integers and this list has at least one element). The function returns 
the "best" sum ie the biggest possible sum of k distances less than or equal to the given limit t, if that sum exists, 
or otherwise nil, null, None, Nothing, depending on the language. In that case with C, C++, D, Dart, Fortran, F#, Go, Julia, 
    ...
    ls
      .reduce(
        (a: number[][], b: number) =>
          a.concat(
            a
              .filter((x: number[]) => x.length < k)
              .map((x: number[]) => x.concat([b]))
          ),
        [[]]
      )
      .filter((x: number[]) => x.length === k)
      .map((x: number[]) => x.reduce((a, b) => a + b))
      .filter((x: number) => x <= t)
