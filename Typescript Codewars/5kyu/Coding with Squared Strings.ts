...
We will use squared strings to code and decode texts. To make things easier we suppose 
that our original text doesn't include the character '\n'.

Coding
Input:
a text t of length l.
Consider the smallest integer n such that n * n be greater or equal to l.
Complete t with the char of ascii code 11 (we suppose that this char is not in our original text) until 
the length of t is n * n.
...
You can see counter-clockwise rotation of 90 degrees: 
http://www.codewars.com/kata/56dbf59b0a10feb08c000227
...
export const code = (s: string): string => {
  ...
  const t = s.padEnd(Math.pow(n, 2), String.fromCharCode(11));

  const arr = t.match(new RegExp(".{1," + n + "}", "g")) || [];
  return arr
    .map((col, colIndex) => arr.map((row) => row[colIndex]).reverse())
    .map((e) => e.join(""))
    .join("\n");
};
...
export const decode = (s: string): string => {
  ...
  return arr
    .map((col, colIndex) =>
      arr.map((row, rowIndex) => row[row.length - 1 - colIndex])
    )
    .flat()
    .join("")
    .trim();
