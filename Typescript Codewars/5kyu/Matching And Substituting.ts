...
Replace the current "Program Title" with the prog argument supplied to your function. Also remove "Title", so in the 
example case "Program Title: Primes" becomes "Program: Ladder".
...
export function change(s: string, prog: string, version: string): string {
  const regExpPhone = new RegExp("\\+1-\\d{3}-\\d{3}-\\d{4}", "gm");
  const regExpVersion = new RegExp("(?<=Version: )(\\d+\\.\\d+)$", "gm");

  if (!regExpPhone.test(s) || !regExpVersion.test(s)) {
    return "ERROR: VERSION or PHONE";
  }

  version =
    String((s.match(regExpVersion) || [])[0]) != "2.0" ? version : "2.0";
