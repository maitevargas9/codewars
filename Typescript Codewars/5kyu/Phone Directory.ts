...
Unfortunately everything is mixed, things are not always in the same order; parts of lines are cluttered with non-alpha-numeric 
characters (except inside phone number and name).
...
Could you help John with a program that, given the lines of his phone book and a phone number num returns a string for this 
...
export const phone = (strng: string, num: string): string => {
  let name: string = "";
  let address: string = "";
  for (let line of strng.split("\n")) {
    if (line.includes(num)) {
      if (name != null) {
        return `Error => Too many people: ${num}`;
      }
      name = (line.match(/<.+>/) || [""])[0].slice(1, -1);
      line = line.replace("<" + name + ">", "").replace(num, "");
      address = line
        .split(/\s+|_+|\/+|;+|,+/)
