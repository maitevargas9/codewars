...
The function will receive the restaurant bill (always a positive number) as an argument. You need to 1) add at least 15% in tip, 
2) round that number up to an elegant value and 3) return it.
...
export function gracefulTipping(bill: number): number {
  ...
  return mult < 1 ? Math.ceil(withTip) : Math.ceil(withTip / mult) * mult;
}
