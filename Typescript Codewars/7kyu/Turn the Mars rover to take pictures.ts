...
Mars rover can face 4 different directions, that would be N, S, E, W. Every time it needs to turn it will call a 
command turn passing the current position it is facing and the position it wants to face. 
...
export function turn(current: Direction, target: Direction): MoveDirection {
  ...
  return directions[(currentIndex + 1) % 4] === target ? "right" : "left";
}
