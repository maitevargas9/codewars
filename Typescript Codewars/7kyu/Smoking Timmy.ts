...
Timothy (age: 16) really likes to smoke. Unfortunately, he is too young to buy his own cigarettes and that's why he has to be extremely 
...
export function startSmoking(bars: number, boxes: number): number {
  ...
  const stubsNeededForNewCigarette = 5;

  let totalCigarettes = (bars * boxesPerBar + boxes) * cigarettesPerBox;
  let totalStubs = totalCigarettes;

  while (totalStubs >= stubsNeededForNewCigarette) {
    const newCigarettes = Math.floor(totalStubs / stubsNeededForNewCigarette);
    totalCigarettes += newCigarettes;
    totalStubs = (totalStubs % stubsNeededForNewCigarette) + newCigarettes;
  }
