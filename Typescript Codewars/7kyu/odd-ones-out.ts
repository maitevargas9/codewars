...
individuals are often mixed with other people and families. However you can distinguish the family they belong to by 
the number on the shirts they wear. As the sheriff's assistant it's your job to find all the odd numbered families and 
...
export function oddOnesOut(nums: number[]) {
  return nums.filter((x) => nums.filter((n) => n == x).length % 2 == 0);
}
