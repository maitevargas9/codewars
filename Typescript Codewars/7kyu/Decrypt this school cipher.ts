...
are not readable by teachers they encrypt text with simple algorythm. Every message contains 
...
E.g. this message:
9Hi Alice4 is encrypted as 4'101''99''105''108''65' '105''72'9
...
export const decrypt = (str: string): string => {
  return (
    str
      .match(/'\d+'|\d| /g)
      ?.map((token) =>
        token.startsWith("'") ? String.fromCharCode(+token.slice(1, -1)) : token
      )
      .reverse()
      .join("") || ""
  );
