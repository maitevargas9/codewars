...
The goal of this kata is to create a very simple ASCII encryption and decryption. The encryption algorithm should shift each character's 
...
export function asciiEncrypt(plaintext: string): string {
  return plaintext
    .split("")
    .map((char, index) => String.fromCharCode(char.charCodeAt(0) + index))
    .join("");
}
