...
Write a function that accepts an array of integers code and a key number. As the result, it should return string containg a 
decoded message from the code.
...
export function decode(code: number[], n: number): string {
  ...
  return arr3.map((x) => String.fromCharCode(x + 96)).join("");
}
