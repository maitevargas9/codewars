...
The accounts of the "Fat to Fit Club (FFC)" association are supervised by John as a volunteered accountant. The association is 
funded through financial donations from generous benefactors. John has a list of the first n donations: [14, 30, 5, 7, 9, 11, 15] 
...
The function new_avg(arr, navg) should return the expected donation (rounded up to the next integer) that will permit to reach 
the average navg.
...
export function newAvg(arr: number[], newavg: number): number {
  let n = Math.ceil(newavg * (1 + arr.length) - arr.reduce((x, y) => x + y, 0));
  if (n < 0) {
