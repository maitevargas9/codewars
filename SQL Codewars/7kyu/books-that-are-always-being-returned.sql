...
-- and aren't currently on-loan (meaning that they do not have records with return_date == null). The result should be 
-- ordered by book_id in descending order.
...
WHERE
    book_id IN (
        ...
        GROUP BY
            book_id
        HAVING
            COUNT(loan_id) = COUNT(return_date)
    )
ORDER BY
    book_id DESC;