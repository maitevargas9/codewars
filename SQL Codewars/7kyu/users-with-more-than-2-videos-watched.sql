-- Description
-- In the online streaming platform users watch various videos and create playlists of their favorite content. 
-- The platform wants to identify engaged users who have explored and watched more than two different videos. 
-- This information will help platform to provide personalized recommendations and enhance the user experience.
-- You are working with PostgreSQL DB and being provided with a table named user_playlist containing the historical 
-- playlist data for users. The table has the following structure:
...
-- You already ordered a pizza that is waiting for you to brighten up the end of a hard day's work but instead you have 
-- to rewrite the query while the pizza is cooling down because damn Postgres does not support such a basic feature.
SELECT
    user_id,
    video_id
FROM
