-- Description
-- In celebration of ABC Kata Solutions' anniversary, we focus on the pillars of an organization: 
-- longest-serving and currently active employees. This SQL challenge involves uncovering a hidden 
-- code derived from the first letters of the names of the top 5 longest-tenured employees who are 
-- still actively contributing to ABC Kata Solutions.
-- Craft an SQL query to discover a hidden code by summing the ASCII values of the first letters of 
-- the last names of the top five oldest (longest-tenured) employees who are still actively contributing 
-- to ABC Kata Solutions.
...
-- Requirements:
-- Identify the Top 5 Oldest Current Employees: Find the five employees with the longest tenure at ABC 
-- Kata Solutions who are still employed (i.e., their left_date is NULL).
...
-- Notes:
-- Let's assume for this task that there are no employees with the same joined_date - tests are written to ensure that 
-- no two employees have the same date of join.
...
-- hidden_code
-- 335
-- From the sorted list of those employees who still working at ABC Kata Solutions, the top 5 oldest employees based 
-- on their join dates are:
...
SELECT
    SUM(c) AS hidden_code
FROM
    (
        SELECT
            ASCII (SPLIT_PART (name, ' ', 2)) AS c
        FROM
            employees
        WHERE
            left_date IS NULL
