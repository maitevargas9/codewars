...
integer >= 0), k (number of towns to visit, k >= 1) and ls (list of distances, all distances are positive or zero integers and this list 
...
limit t, if that sum exists, or otherwise nil, null, None, Nothing, depending on the language. In that case with C, C++, D, Dart, Fortran, 
...
function chooseBestSum(t, k, ls) {
  return (
    ls
      .reduce(
        (a, b) =>
          a.concat(a.filter((x) => x.length < k).map((x) => x.concat([b]))),
        [[]]
      )
      .filter((x) => x.length === k)
