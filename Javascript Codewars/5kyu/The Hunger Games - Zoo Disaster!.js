...
var whoEatsWho = function (zoo) {
  ...
  function canEat(eater, food) {
    return rules[eater] && rules[eater].includes(food);
  }
  ...
  while (ate) {
    ...
    for (let i = 0; i < animals.length; i++) {
      ...
      if (i > 0 && canEat(eater, animals[i - 1])) {
        result.push(`${eater} eats ${animals[i - 1]}`);
        animals.splice(i - 1, 1);
        ate = true;
        break;
      }

      if (i < animals.length - 1 && canEat(eater, animals[i + 1])) {
        result.push(`${eater} eats ${animals[i + 1]}`);
        animals.splice(i + 1, 1);
        ate = true;
        break;
