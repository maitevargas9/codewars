...
decompose this number as a sum of rationals with numerators equal to one and without 
...
The rational given to decompose could be greater than one or equal to one, in which case the first "fraction" will be an integer (with an 
...
function decompose(n) {
  const target = Number.isNaN(Number(n))
    ? Number(n.split("/")[0]) / Number(n.split("/")[1])
    : Number(n);
  const divs = [];
