...
one another box (which, in turn, can contain at most one another box, and so on).
More formally, your task is to check whether there is such sequence of n different numbers pi (1 ≤ pi ≤ n) that 
for each 1 ≤ i < n the box number pi can be put into the box number pi+1.
...
function boxesPacking(length, width, height) {
  let element = length.reduce(
    (x, y, z) => (x.push([y, width[z], height[z]].sort((a, b) => b - a)), x),
    []
  );
  element.sort((a, b) => b[0] - a[0] || b[1] - a[1] || b[2] - a[2]);
  return element.every(
    (x, y, z) => !y || (x[0] < (y = z[y - 1])[0] && x[1] < y[1] && x[2] < y[2])
  );
}
