...
In this lesson, we learn three special wildcard "?", "*", "+" and curly braces "{}". They are related to the number of matching 
characters.

First of all, we look at the "{}", and its usage is:{min,[max]}. It means matching a range: at least min characters, up to max 
characters. Let's see some examples:
...
We can see that the regular expression always matches the maximum value in the range. max can be ommited, at this time the 
regular expression will match the maximum possible number. like this:
...
If the comma is omitted, the regular expression will match the string that is equal to the number(Equivalent to max and min are 
equal). like this:
...
This extended training kata will help you understand the usage of "{}". Coding 3min : Regular expression compression But it may 
...
See the example above, we can conclude that the "personality" of regular expression is "greedy". It always tries to match the 
longest results. Sometimes we don't get the results we want. Look at the following example:
...
Oh no! This is not what I want. The "greedy" of regular expression is the "original sin". It matches the longest string: the 
string itself. How to solve this problem? Very simple, another usage of the "?" can make the regular expression become "lazy". 
See examples:
...
Wow! We got the desired results. So we should know: "+?" and "*?" can make regular expressions become "lazy". It always matches 
the shortest string. Of course, the example above uses "*?" Is not suitable. Let's see what results when we use "*?":
...
|100001|,|11| |1,1|222222|1,222,3333,1|232|1,1|4441
 match-1  2     3               4           5
The second match is "11", the ".*" is actually matched to the empty string, causing the match to the confusion. So, we choose to 
use wildcards according to the actual situation. As far as possible to make the regular expression more clear.

Ok, lesson is over. let's us do some task.

Task

This time you do not need coding in function. You just need to write a regular expression that matches the specified numeric 
string. I've given the regular expression name: "regex", please don't modify the name.
