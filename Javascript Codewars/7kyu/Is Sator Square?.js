...
strange stone tablets. Noticing they were carved with letters in a square pattern, he wisely kept them in case some might be special.
...
function isSatorSquare(tablet) {
  ...
  for (let i = 0; i <= Math.floor(l / 2); i++) {
    ...
    let across = tablet[i].join("");
    let down = tablet.map((row) => row[i]).join("");
    let up = tablet
      .map((row) => row[j])
      .slice()
      .reverse()
      .join("");
    let reverse = tablet[j].slice().reverse().join("");

    if (!(across === down && across === up && across === reverse)) {
      return false;
    }
  }
