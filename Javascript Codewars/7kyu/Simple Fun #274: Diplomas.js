...
Task
In 10 years of studying, Alex has received a lot of diplomas. He wants to hang them on his wall, but unfortunately 
it's made of stone and it would be too difficult to hammer so many nails in it.
Alex decided to buy a square desk to hang on the wall, on which he is going to hang his diplomas. Your task is to 
find the smallest possible size of the desk he needs.
...
function diplomas(h, w, n) {
  ...
  let left = 0;
  let right = Math.max(h, w) * n;

  while (left < right) {
    const mid = Math.floor((left + right) / 2);
    const fit = Math.floor(mid / h) * Math.floor(mid / w);
