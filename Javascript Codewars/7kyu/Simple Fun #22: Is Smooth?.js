...
The first and the last elements of arr are equal to 7, and its middle also equals 2 + 5 = 7. Thus, the array is smooth and the
output is true.
...
function isSmooth(arr) {
  let a = arr[Math.ceil((arr.length - 1) / 2)];
  let b = arr[Math.floor((arr.length - 1) / 2)];
  return a === arr[0] && b === arr[arr.length - 1]
    ? true
    : a + b == arr[0] && a + b === arr[arr.length - 1]
    ? true
    : false;
}
