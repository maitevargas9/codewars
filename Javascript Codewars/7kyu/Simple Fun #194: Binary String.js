...
For example, applying the Flip operation to the 4th character of string "1001010" produces the "1000101" string, since all characters from 
the 4th to the 7th are flipped.
...
function binStr(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    const bit = flipped ? (s[i] === "1" ? "0" : "1") : s[i];
    if (bit === "1") {
      flips++;
      flipped = !flipped;
    }
