...
Oh no! Marty McFly was taking the DeLorean for a drive and accidentally time travelled to approximately 50 years in 
the past.

The last thing he remembers is Doc Brown shouting "Marty, you must only attempt to travel back on the same day and
date as you arrive, otherwise the space-time continuum will unravel! The month doesn't matter, but if you arrive on Monday 1st, you must leave on a Monday 1st or the consequences will be disastrous. The flux capacitor is only going to last until the end of the year, so if you can't travel before then, you'll be stuck in the past forever!".
...
If there is no matching day/date that year, Marty is stuck in the past and you will need to return the following 
string: "Doc, I can't get back to the future!"
...
function backToTheFuture(str) {
  ...
  const match = alignedMonths[elem[2]];

  if (match && elem[1] <= match.maxDays) {
    return `I'm leaving here on ${elem[0]} ${elem[1]} ${match.outbound}!`;
  }
