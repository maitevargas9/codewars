...
Maybe you don't often use random() when you're dealing with Kata, but every qualified Kata can not be missing random(). Because 
random testcase is the most basic anti cheating measure. A variety of testcases also help authors identify potential problems. If 
you also want to create a new Kata and become an author, please follow me to learn about random().
...
85
0
A number multiply with random(), and then use the Math.floor(~~ is a simplified form of math.floor) to get an integer. The range 
is 0 to number-1.
...
What should we do if we want to get a character from some of the characters that do not have a regular character? For example, 
we want to randomly generate a character from aeiou, we can do this:
...
function rndCode() {
  let x1 = "ABCDEFGHIJKLM";
  let x2 = "0123456789";
  let x3 = "~!@#$%^&*";
  let str = "";
  for (let i = 1; i <= 8; i++) {
    if (i === 1 || i === 2) {
      str += x1[~~(x1.length * Math.random())];
    }
    if (i > 2 && i < 7) {
      str += x2[~~(x2.length * Math.random())];
    }
    if (i > 6) {
      str += x3[~~(x3.length * Math.random())];
    }
  }
  return str;
}
