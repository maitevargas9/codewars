...
Math.min()
Math.abs()
Their usage is very simple: for the given parameters, return the maximum value, minimum value and absolute value. Here we use 
some examples to understand their usage:
var a=1,b=2,c=-1
...
We can see: For max() and min(). If it has only one parameter, method returns the parameter itself; If it has many parameters, 
method will return the maximum / minimum values; For abs(). No matter how many parameters, it returns only the absolute value of 
the first parameter.

max() and min() can accept an array as a parameter by using spread operator (...). The following example is a classic usage, to 
calculate the maximum/minimum values of the array:
var arr=[1,2,3,4,5];
...
Coding in function maxMin. function accept 2 parameter arr1 and arr2. They are two number array and have the same number of 
elements.
...
[1,3,5]
 | | |   --->  8, 5, 2
[9,8,7]
Please note that the difference is positive. Such as the above 1 and 9, the difference should be 8, not -8. I think abs() can 
help you get the correct result ;-)
...
function maxMin(arr1, arr2) {
  const arr = arr2.map((x, y) => x - arr1[y], 0);
  return [
    Math.max(...arr.map((x) => Math.abs(x))),
    Math.min(...arr.map((x) => Math.abs(x)))
  ];
}
