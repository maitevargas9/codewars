...
In order for a bill to pass, it must have a majority vote, meaning that more than half of all members must approve of a bill before it is 
passed. The "Conservatives" and "Reformists" always vote the same as other members of thier parties, meaning that all the members of each 
party will all vote yes, or all vote no.
...
In each test case the makeup of the Parliament will be different. In some cases your party may make up the majority of parliament, and in 
others it may make up the minority. If your party is the majority, you may find that you do not neeed any independants to vote in favor of 
your bill in order for it to pass. If your party is the minority, it may be possible that there are not enough independants for your bill 
to be passed. If it is impossible for your bill to pass, return -1.
...
function passTheBill(
  totalMembers,
  conservativePartyMembers,
  reformistPartyMembers
) {
  const majority = Math.floor(totalMembers / 2) + 1;
  const independents =
    totalMembers - conservativePartyMembers - reformistPartyMembers;
  const needed = majority - conservativePartyMembers;

  return needed <= 0 ? 0 : needed > independents ? -1 : needed;
}
