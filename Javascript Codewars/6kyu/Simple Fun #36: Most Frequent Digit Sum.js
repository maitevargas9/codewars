...
functions to numbers, so given the number n, you decide to build a decreasing sequence of numbers: n, step(n), step(step(n)), 
etc., with 0 as the last element.
...
Now you're curious as to which number appears in the new sequence most often. If there are several answers, return the maximal 
one.
...
function mostFrequentDigitSum(n) {
  const sum = (val) => [...`${val}`].reduce((pre, curr) => pre + +curr, 0);
  const obj = {};
  for (let i = n; i > 0; i -= sum(i)) {
    obj[sum(i)] = -~obj[sum(i)];
  }
  return +Object.keys(obj).reduce((pre, curr) =>
    obj[pre] > obj[curr] ? pre : curr
  );
}
