...
Task
Given a rectangular matrix of integers, check if it is possible to rearrange its rows in such 
a way that all its columns become strictly increasing sequences (read from top to bottom).
...
function rowsRearranging(matrix) {
  return matrix
    .slice()
    .sort((a, b) => a[0] - b[0])
    .every((x, y, z) => y == 0 || x.every((i, j) => i > z[y - 1][j]));
}
