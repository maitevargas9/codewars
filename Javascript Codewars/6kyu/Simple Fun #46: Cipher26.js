...
The number corresponding to the ith letter of the encrypted message is then equal to the sum of numbers corresponding to the 
first i letters of the initial unencrypted message modulo 26.
...
function cipher26(message) {
  let alph = [...Array(26)].map((_, i) => String.fromCharCode(97 + i));
  return [...message].reduce(
    (acc, c, i) =>
      acc +
      alph[(alph.indexOf(c) - (i && alph.indexOf(message[--i])) + 26) % 26],
    ``
  );
}
