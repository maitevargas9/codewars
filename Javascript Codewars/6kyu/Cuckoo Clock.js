...
At the beginning of each hour (1-12), it chimes out the hour. Given the current time and a positive integer n, determine 
the time when the cuckoo bird has chimed n times.
...
and once each at "05:15", "05:30", "05:45". At this point it has chimed 16 times, so the 19th chime occurs when it chimes 
6 times at "06:00".
...
function cuckooClock(inputTime, chimes) {
  ...
  while (true) {
    ...
    if (m === 0) c = hourChimes(h);
    else if (m === 15 || m === 30 || m === 45) c = 1;
